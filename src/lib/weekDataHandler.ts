...
export const getWeekData = (uid: string, week: number) => {
    const newStartDate = getStartOfWeek(new Date(today.setDate(today.getDate() + 7 * week)));
    const newEndDate = getEndOfWeek(new Date(today.setDate(today.getDate() + 7 * week)));
